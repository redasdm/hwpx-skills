...
한/글에서 열리는 것과 표를 편집할 수 있는 것은 별개다. XML이 well-formed이고 쪽수 가드가 통과해도 표의 내부 좌표가 깨져 있으면 셀 클릭·텍스트 입력·행 조정 시 한/글이 비정상 종료할 수 있다.

- 모든 `<hp:tbl>`에 대해 `rowCnt`/`colCnt`, 실제 `<hp:tr>`/`<hp:tc>` 배열, `<hp:cellAddr>`, `<hp:cellSpan>`을 함께 점검한다.
- `rowAddr`/`colAddr`는 0부터 시작하는 논리 격자 좌표다. `0 <= rowAddr < rowCnt`, `0 <= colAddr < colCnt`, `rowAddr + rowSpan <= rowCnt`, `colAddr + colSpan <= colCnt`를 만족해야 한다.
- `rowAddr`가 실제 논리 행 순서와 일치하는지 확인한다. 예를 들어 `rowCnt="4"`인 표의 마지막 행에 `rowAddr="5"`가 있으면 XML 파싱과 렌더링이 되더라도 편집 시 실패할 수 있다.
- `cellSpan`을 반영한 점유 격자를 만들어 셀 간 겹침·빈 칸·범위 밖 좌표를 검사한다. `cellSz`/`cellMargin`과 표 전체 폭·높이도 레퍼런스와 비교한다.
- `<hp:linesegarray>`는 파생된 줄 배치 캐시일 뿐이다. 캐시를 삭제하거나 다시 만드는 것만으로 잘못된 `cellAddr`/`cellSpan`을 고칠 수 없다. 표 모델 무결성을 별도로 검사한다.
- 변경한 표마다 한/글에서 실제 셀을 선택해 임시 표식 텍스트를 입력·삭제하고 별도 사본에 저장하는 편집 스모크 테스트를 수행한다. 단순히 열기·다시 저장하는 테스트만으로는 부족하다.
...
27. **표 좌표 정합성**: 모든 표에서 `rowCnt`/`colCnt`와 `hp:cellAddr`/`hp:cellSpan`의 논리 격자를 검증한다. 좌표는 0부터 시작하고 범위를 벗어나거나 겹치거나 비는 셀이 없어야 한다. `linesegarray`만 삭제·재생성하는 것은 이 검사를 대체하지 못한다.
28. **한컴 편집 스모크 테스트**: 변경한 표의 셀을 실제로 선택해 임시 텍스트를 입력·삭제한 뒤 별도 사본을 저장한다. 열기·다시 저장만 성공한 경우 표 편집 가능성을 확인한 것으로 간주하지 않는다.
